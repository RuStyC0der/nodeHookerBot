...
Example:
curl "http://${domain}?n=yournickname&msg=test"

Use case:
bash ./<very_long_task_or_script>.sh && curl "http://${domain}?n=yournickname&msg=task_finished" (curl will be executed only if previous task was finished with 0 status code)
bash ./<very_long_task_or_script>.sh ; curl "http://${domain}?n=yournickname&msg=task_finished" (curl will be executed even if previous task was finished with non-zero status code)
